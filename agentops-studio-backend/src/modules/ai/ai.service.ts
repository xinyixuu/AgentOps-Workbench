...
@Injectable() // 表示这个类可以被Nest注入
export class AiService {
  constructor(
    private prisma: PrismaService,
    private configService: ConfigService,
    private ragService: RAGService,
    @Inject(forwardRef(() => WorkflowExecutorService)) // 延迟解析，两个module之前可以存在循环依赖关系
  ...
  async streamRun(userId: string, streamRunDto: StreamRunDto, res: Response) {
    ...
    try {
      ...
      const sseSubject = new Subject<any>(); // 类似一个事件广播器
  ...
  private async resolveWorkflowId(userId: string, appId: string, workflowId?: string): Promise<string> {
    if (workflowId) {
      ...
      if (workflow.application.userId !== userId) {
        throw new Error('You do not have permission to run this workflow');
      }
  ...
  async chat(userId: string, chatDto: ChatDto, res: Response) {
    ...
    try {
      ...
      if (knowledgeBaseId) {
        try {
          const retrieval = await this.ragService.retrieve(message, knowledgeBaseId, { topK: 5, userId });
          references = retrieval.documents;
