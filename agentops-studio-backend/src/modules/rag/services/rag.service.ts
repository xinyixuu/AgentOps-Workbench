...
export interface RetrievalDiagnostics {
  ...
  totalChunks: number;
  chunksWithEmptyEmbedding: number;
  candidateCount: number;
  passedThresholdCount: number;
  filteredBelowThresholdCount: number;
  maxSimilarity: number;
  noRecallReason: string | null;
}
...
export class RAGService {
  ...
  // 检索
  async retrieve(query: string, knowledgeBaseId: string, options: RetrieveOptions | number = 5): Promise<RetrievalResult> {
    const retrieveOptions = typeof options === 'number' ? { topK: options } : options;
    ...
    if (retrieveOptions.userId && knowledgeBase.userId !== retrieveOptions.userId) {
      throw new BadRequestException('You do not have permission to retrieve this knowledge base');
    }

    const effectiveTopK = retrieveOptions.topK || knowledgeBase.topK || 5;
    const effectiveThreshold = retrieveOptions.similarityThreshold ?? knowledgeBase.similarityThreshold ?? 0;
    ...
    const passedThreshold = sortedChunks.filter((chunk) => chunk.similarity >= effectiveThreshold);
    const documents = passedThreshold.slice(0, effectiveTopK) as RetrievedChunk[];
    ...
    return {
      ...
      documents,
      diagnostics: {
        queryEmbeddingEmpty: queryVector.length === 0,
        totalChunks: allChunks.length,
        chunksWithEmptyEmbedding,
        candidateCount: scoredChunks.length,
        passedThresholdCount: passedThreshold.length,
        filteredBelowThresholdCount: sortedChunks.length - passedThreshold.length,
        maxSimilarity: sortedChunks[0]?.similarity ?? 0,
        noRecallReason: this.getNoRecallReason({
          queryEmbeddingEmpty: queryVector.length === 0,
          totalChunks: allChunks.length,
          chunksWithEmptyEmbedding,
          passedThresholdCount: passedThreshold.length,
  ...
  private getNoRecallReason(input: {
    ...
    if (input.passedThresholdCount > 0) return null;
    if (input.queryEmbeddingEmpty) return 'query_embedding_empty';
    if (input.totalChunks === 0) return 'knowledge_base_has_no_chunks';
    if (input.chunksWithEmptyEmbedding >= input.totalChunks) return 'chunk_embeddings_empty';
    if (input.effectiveThreshold > 0) return 'similarity_threshold_too_high';
