...
const Debug: React.FC = () => {
  ...
  const handleRunWorkflow = async () => {
    ...
    try {
      ...
      const appendTraceEvent = (eventType: string, payload: any) => {
        setTraceEvents(prev => [
          ...prev,
          {
            eventType,
        ...
        if (event.type === 'event') {
          try {
            ...
            if (data.type === 'run_started') {
              runIdFromStream = data.data?.runId || ''
              setCurrentRunId(runIdFromStream)
              appendTraceEvent('run_started', data.data)
            } else if (data.type === 'step_started') {
              appendTraceEvent('step_started', data.data)
              const { nodeId, stepId, nodeType } = data.data
              setNodeStates(prev => ({
                ...prev,
                [nodeId]: { nodeId, stepId, nodeType, status: 'running' },
              }))
            } else if (data.type === 'step_finished' || data.type === 'step_skipped') {
              appendTraceEvent(data.type, data.data)
              const { nodeId, stepId, nodeType, status, output, error, duration } = data.data
              setNodeStates(prev => ({
                ...prev,
                [nodeId]: { nodeId, stepId, nodeType, status, output, error, duration },
              }))
            } else if (data.type === 'tool_call' || data.type === 'rag_retrieval') {
              appendTraceEvent(data.type, data.data)
            } else if (data.type === 'node_status') {
              const { nodeId, status, output, error } = data.data
              setNodeStates(prev => ({
                ...prev,
                [nodeId]: { ...prev[nodeId], nodeId, status, output, error },
              }))
            } else if (data.type === 'done') {
              if (data.data?.runId) {
                runIdFromStream = data.data.runId
  ...
  const traceEventLabel = (eventType: string) => {
    switch (eventType) {
      case 'run_started': return 'Run 开始'
      case 'run_finished': return 'Run 结束'
      case 'step_started': return 'Step 开始'
      case 'step_finished': return 'Step 结束'
      case 'step_skipped': return 'Step 跳过'
      case 'tool_call': return '工具调用'
      case 'rag_retrieval': return 'RAG 召回'
      case 'token': return 'Token 输出'
      case 'error': return '异常'
      default: return eventType
    }
  }

  const tracePayloadPreview = (payload: any) => {
    if (!payload) return ''
    if (payload.message) return payload.message
    if (Array.isArray(payload.documents)) return `${payload.nodeId || 'RAG'} · 召回 ${payload.documents.length} 个片段`
    if (payload.output && payload.skillId) return `${payload.skillId} · 工具调用完成`
    if (payload.nodeId && payload.status) return `${payload.nodeId} · ${payload.status}`
    ...
    <div className="debug-page">
      ...
      {activeTab === 'workflow' && (
        <div className="debug-workflow-card">
          ...
          {/* Trace 时间线 */}
          {(currentRunId || runTrace || timelineEvents.length > 0) && (
            <div className="debug-trace-panel">
              <div className="debug-trace-head">
                <div>
                  <div className="debug-trace-title">Run / Step Trace</div>
                  <div className="debug-trace-subtitle">
                    {currentRunId || runTrace?.id ? `Run ID: ${currentRunId || runTrace?.id}` : '等待运行记录'}
                    {runTrace?.duration ? ` · ${runTrace.duration}ms` : ''}
                  </div>
                </div>
                <span className={`debug-trace-status debug-trace-status--${runTrace?.status || wfStatus}`}>
                  {runTrace?.status || wfStatus}
                </span>
              </div>

              {persistedSteps.length > 0 && (
                <div className="debug-trace-steps">
                  {persistedSteps.map((step) => (
                    <div key={step.id} className="debug-trace-step">
                      <div className="debug-trace-step-main">
                        {nodeStatusIcon(step.status)}
                        <span className="debug-trace-step-name">{step.label || step.nodeId}</span>
                        <span className="debug-trace-step-type">{step.nodeType}</span>
                      </div>
                      <div className="debug-trace-step-meta">
                        <span>{nodeStatusLabel(step.status)}</span>
                        {typeof step.duration === 'number' && <span>{step.duration}ms</span>}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {timelineEvents.length > 0 && (
                <div className="debug-trace-events">
                  {timelineEvents.map((event) => (
                    <div key={event.id || `${event.sequence}-${event.eventType}`} className="debug-trace-event">
                      <span className="debug-trace-event-seq">#{event.sequence}</span>
                      <span className={`debug-trace-event-type debug-trace-event-type--${event.eventType}`}>
                        {traceEventLabel(event.eventType)}
                      </span>
                      <span className="debug-trace-event-preview">
                        {tracePayloadPreview(event.payload)}
                      </span>
                    </div>
                  ))}
                </div>
